...
| `threshold_ladder_10_40bp` | `16.56%` | `0.96` | `-23.29%` | `0.82` | `52.10` |
...
The `threshold_ladder_5_20bp` version uses an explicit rule: `5bp` expected daily edge maps to `25%` exposure, `10bp` to `50%`, `15bp` to `75%`, and `20bp` or more to `100%`. The stricter `threshold_ladder_10_40bp` version uses the same exposure steps but requires `10bp`, `20bp`, `30bp`, and `40bp` edges. These are easy to explain, but the current model can still spend many days at full size because its estimated edges are often large.
